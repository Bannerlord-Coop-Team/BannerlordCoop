...
if(Test-Path (${BaseDir} + $config.modsDir))
{
    ...
    Get-ChildItem -Path "${ModSourceDir}" -Include *.dll -Recurse -ErrorAction Ignore | Where { $_.PSIsContainer -eq $false } | Copy-Item -Destination "${ModDir}\bin\Win64_Shipping_Client"
