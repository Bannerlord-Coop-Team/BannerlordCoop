...
2. Prepare a Bannerlord installation with a **matching DEBUG build** of Coop, Common, GameInterface and the other mod assemblies. The bridge is compiled out in Release. Launch tools never build or deploy. The separately authorized, opt-in `deploy_mod` tool with `configuration: "Debug"` can update an existing installation from a trusted checkout; see [explicit deployment and recovery](../../doc/automated-testing/mcp-deployment.md). It does not install game dependencies.
...
4. Wait for each client `readyForCampaignTests`. This also attempts the startup popup dismissal described below; inspect `instance.startupPopup` separately from `reached`. The server's `connectedPlayerCount` and `connectedControllerIds` provide additional join diagnostics. A join acknowledgment is not a completed connection.
...
`ui_layers` discovers bounded client layers without traversing widget descendants. Pass a fresh returned opaque handle as optional `ui_inspect.layer` to inspect its complete context and relevant upper blocking domains. Carry the same layer when paging a selected snapshot. Discovery and each explicitly selected inspection first require loaded status `uiCapability: bounded-ui-layers-v1` under the owned-instance gate. Missing/wrong capability or failed status sends no UI request and never falls back to the whole tree. A malformed layer is rejected before routing. Omit `layer` for the unchanged legacy `{ snapshot, offset }` payload, with no capability probe.

Snapshots and handles expire or invalidate when UI/layers change. Truncated or incomplete scopes are inspection-only. On the matching bounded-layer bridge, `ui_action` click/toggle uses exact native `ButtonWidget.HandleClick` semantic activation, not physical press/release, focus or keyboard input. Selected scopes advertise only click/toggle; all-layer snapshots retain legacy non-button actions. Use only advertised actions, re-inspect and capture afterward, and never retry uncertain mutations. `ui_action` parameters, validation and routing are unchanged here. This MCP update does not install that bridge or prove native callback safety. See [MCP Gauntlet menu automation](../../doc/automated-testing/mcp-ui.md) for legacy workflow limits and `/autoconnect` endpoint syntax. Never use Apply during reversible menu checks.
...
### War Sails startup popup

A client `readyForCampaignTests` wait, after campaign readiness on `NavalMapScreen`, observes for up to three seconds within the caller's remaining deadline and tries one native Continue click on **Call of the Oceans**. No launch-time click, server action, OS input or gameplay-check change is made. `wait_for_state` is therefore no longer advertised as read-only; other state waits remain observational.

This requires the loaded bridge's `bounded-ui-layers-v1` capability, not present in this checkout's legacy DEBUG bridge source. It uses a fresh QueryManager layer handle and a complete single-page scope, matching the English title, vanilla description prefix and visible `SingleQueryOkButton` with text `Continue`. The button must be an enabled, hit-testable exact native ButtonWidget advertising click; the bridge revalidates visibility, coverage and snapshot identity before dispatch. Localized text, Troubled Waters, character creation and other queries are not dismissed. Incomplete or larger-than-one-page scopes fail closed.

`instance.startupPopup.outcome` is independent of campaign readiness: `dismissed` requires a successful dispatch plus a fresh QueryManager observation with inactive layer, zero roots and zero input mask. `not_present` means absent when observed, not proof it cannot appear later. `unsupported`, `not_applicable`, `not_actionable`, `not_observed` and `inspection_failed` mean no click was attempted. `unconfirmed` means a click was attempted but closure was not confirmed; inspect its `action`/`error`, fresh UI and a screenshot instead of blindly clicking again. The per-client/run attempt is consumed before sending and is never replayed, even by repeated waits. Read-only observation can be repeated when no click has been attempted. Automatic UI responses and the last popup outcome are retained with run artifacts. No screenshot is automatically requested.
...
- V1 uses the game's existing auto-start and connection configuration. It does not change ports/passwords, bypass startup errors, implicitly run builds/deployments, hotload campaigns, or coordinate background joins. Explicit `deploy_mod` is a separate tool, never part of launch.

## Explicit deployment

`deploy_mod` requires `solution_path` (an absolute `source/Coop.sln`) and an existing `profile`. Optional `configuration` defaults to `Release`; only `Release` and `Debug` are accepted. Specify `Debug` for live testing: a successful Release deployment has no live-test bridge and remains blocked by launch preflight. The selected configuration is returned in the report and durable manifest. It requires an opt-in `deployment` configuration with a durable non-Temp backup root and Windows MSBuild path. It refuses active runs/games, rebuilds with deployment targets disabled, and applies only a bounded mod-output/UI set after fresh verified per-file backups. Inspect its returned state, manifest and unresolved restoration list. No full-module or DedicatedServer backup is made. XML/config/optin files are untouched unless a complete replacement `subModuleXml` was deliberately configured. Read [schema, permissions, safety and recovery](../../doc/automated-testing/mcp-deployment.md) before enabling it.
