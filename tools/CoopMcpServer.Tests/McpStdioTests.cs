...
public sealed class McpStdioTests
{
    ...
    public async Task InitializeAndToolsListWorkWithoutGameOrConfiguredProfiles()
    {
        ...
        try
        {
            ...
            var deploySchema = tools.Single(t => t.Name == "deploy_mod").JsonSchema;
            Assert.Equal(new[] { "configuration", "profile", "solution_path" }, deploySchema.GetProperty("properties").EnumerateObject().Select(p => p.Name).Order());
            Assert.Equal(new[] { "profile", "solution_path" }, deploySchema.GetProperty("required").EnumerateArray().Select(p => p.GetString()).Order());
            Assert.Equal("Release", deploySchema.GetProperty("properties").GetProperty("configuration").GetProperty("default").GetString());
            var refused = await client.CallToolAsync("deploy_mod", new Dictionary<string, object> { ["profile"] = "missing", ["solution_path"] = "source/Coop.sln" }, cancellationToken: timeout.Token);
