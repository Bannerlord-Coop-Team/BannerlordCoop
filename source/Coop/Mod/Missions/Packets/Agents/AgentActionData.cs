    ...
    public class AgentActionData
    {
        ...
        public void Apply(Agent agent)
        {
            ...
            if (agent.GetCurrentAction(0) == ActionIndexCache.act_none || agent.GetCurrentAction(0).Index != Action0Index)
            {
                ...
                agent.SetActionChannel(0, ActionIndexCache.Create(actionName1), additionalFlags: (ulong)Action0Flag, startProgress: Action0Progress);
            ...
            if ((int)Action1CodeType >= (int)Agent.ActionCodeType.DefendAllBegin && (int)Action1CodeType <= (int)Agent.ActionCodeType.DefendAllEnd)
            ...
            if ((Agent.ActionCodeType)Action1CodeType != Agent.ActionCodeType.BlockedMelee)
            {
                ...
                if (agent.GetCurrentAction(1) == ActionIndexCache.act_none || agent.GetCurrentAction(1).Index != Action1Index)
                {
                    ...
                    agent.SetActionChannel(1, ActionIndexCache.Create(actionName2), additionalFlags: (ulong)Action1Flag, startProgress: Action1Progress);
