...
namespace Coop.Game.Persistence.World
{
    public class WorldState : RailState<WorldState>
    {
        ...
        protected override void ApplyMutableFrom(WorldState source, uint uField)
        {
            ...
            if (flag.HasFlag(Field.TimeControlMode)) Data.TimeControlMode = source.Data.TimeControlMode;
        }

        protected override uint CompareMutableData(WorldState other)
        {
            return (uint)
                (Data.TimeControlMode == other.Data.TimeControlMode ? Field.None : Field.TimeControlMode);
        }
        protected override void DecodeMutableData(RailBitBuffer buffer, uint uField)
        {
            Field flag = (Field)uField;
            if (flag.HasFlag(Field.TimeControlMode)) Data.TimeControlMode = (CampaignTimeControlMode) buffer.ReadByte();
        }
        protected override void EncodeMutableData(RailBitBuffer buffer, uint uField)
        {
            Field flag = (Field)uField;
            if (flag.HasFlag(Field.TimeControlMode)) buffer.WriteByte(Convert.ToByte(Data.TimeControlMode));
        }
        #endregion
        #region Immutable
