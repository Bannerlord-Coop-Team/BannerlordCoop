...
public class BattleBlockingSyncTests : MissionTestEnvironment
{
    ...
    public void PreDisplayPoll_MountedGuardActionCatchingUp_BroadcastsVisibleDirection()
    {
        RunScenario("owner", context =>
        {
            ...
            AgentActionData requestedRight = Assert.Single(
                Assert.Single(
                    context.Network.NetworkSentPackets
                        .GetPackets<AgentActionPacket>())
                    .Actions);
            Assert.Equal(202, requestedRight.Action1Index);
            ...
            AgentActionPacket packet = Assert.Single(
                context.Network.NetworkSentPackets
                    .GetPackets<AgentActionPacket>());
            AgentActionData realizedRight = Assert.Single(packet.Actions);
            Assert.Equal(203, realizedRight.Action1Index);
            Assert.Equal(0.15f, realizedRight.Action1Progress);
            Assert.Equal(Agent.GuardMode.Right, realizedRight.GuardMode);
            Assert.Equal(3L, Assert.Single(packet.Sequences));
        });
    }
    ...
    public void MountedPuppet_AcceptedDirectionAction_RetriesUntilRawActionChanges()
    {
        RunScenario("peer", context =>
        {
            ...
            int guardCommandsBeforeTransition =
                puppetMirror.SetWeaponGuardCalls;
            int guardResetsBeforeTransition =
            ...
            ownerMirror.Action1Direction =
                Agent.UsageDirection.DefendRight;
            puppetMirror.AcceptedSetActionChannelDeferralsRemaining = 3;
            ...
            Assert.Equal(
                guardCommandsBeforeTransition + 1,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
                guardResetsBeforeTransition + 1,
                puppetMirror.ResetGuardCalls);
            Assert.Equal(
                new[]
                {
            ...
            Assert.True(
                (puppetMirror.LastSetActionFlags
                    & AnimFlags.anf_restart) != (AnimFlags)0uL);
            Assert.Equal(
                guardCommandsBeforeTransition + 2,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
                guardResetsBeforeTransition + 1,
                puppetMirror.ResetGuardCalls);
            ...
            Assert.Equal(
                actionCommandsBeforeTransition + 4,
                puppetMirror.SetActionChannelCalls);
            Assert.Equal(
                guardCommandsBeforeTransition + 3,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
                guardResetsBeforeTransition + 1,
                puppetMirror.ResetGuardCalls);
    ...
    [Fact]
    public void MountedPuppet_OppositeGuardTransition_ClearsRetainedNativeSibling()
    {
        RunScenario("peer", context =>
        {
            var agentId = Guid.NewGuid();
            ...
            puppetMirror.RetainedNativeActionArmed = true;
            puppetMirror.RetainedNativeActionChannel = 1;
            puppetMirror.RetainedNativeActionIndex = 3078;
            puppetMirror.RetainedNativeActionType =
                Agent.ActionCodeType.DefendLeft1h;
            ...
            Assert.False(puppetMirror.ResumeRetainedNativeAction());
            Assert.Equal(0, puppetMirror.RetainedNativeActionResumeCalls);
            Assert.Equal(3070, puppetMirror.Action1Index);
            Assert.Equal(
    ...
    public void PreMissionTick_MountedRealizedDirection_RecommandsBeforeDisplay()
    {
        RunScenario("peer", context =>
        {
            ...
            int guardCommandsBeforeMissingAction =
                puppetMirror.SetWeaponGuardCalls;
            int guardResetsBeforeMissingAction =
                puppetMirror.ResetGuardCalls;
            context.Component.AgentActionHandler.ApplyRemoteGuardStates();

            Assert.Equal(Agent.GuardMode.Right, puppetMirror.GuardMode);
            Assert.Equal(
                guardCommandsBeforeMissingAction + 1,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
                guardResetsBeforeMissingAction + 1,
                puppetMirror.ResetGuardCalls);
            Assert.Equal(
                actionCommandsBeforeDisplay,
                puppetMirror.SetActionChannelCalls);
            ...
            puppetMirror.Action1Direction =
                Agent.UsageDirection.DefendRight;
            int guardCommandsBeforeStableAction =
                puppetMirror.SetWeaponGuardCalls;
            int guardResetsBeforeStableAction =
                puppetMirror.ResetGuardCalls;
            context.Component.AgentActionHandler.ApplyRemoteGuardStates();

            Assert.Equal(
                guardCommandsBeforeStableAction,
                puppetMirror.SetWeaponGuardCalls);

            controller.OnMissionTick(0.1f);

            Assert.Equal(
                guardCommandsBeforeStableAction + 1,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
                guardResetsBeforeStableAction,
                puppetMirror.ResetGuardCalls);
            Assert.Equal(
            ...
            Assert.Equal(
                ...
                puppetMirror.SetActionChannelCalls);
            Assert.Equal(
                guardCommandsBeforePeriodicDisplacement + 2,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
                guardResetsBeforePeriodicDisplacement,
                puppetMirror.ResetGuardCalls);
    ...
    [Fact]
    public void MountedRealizedDirection_BeforeGuardTick_DoesNotRepeatAcceptedPresentation()
    {
        RunScenario("peer", context =>
        {
            var controller = context.Instance.Container.Resolve<CoopBattleController>(
    ...
    [Fact]
    public void MissionTicks_MountedSameDirectionGuard_RefreshesAfterReactionEnds()
    {
        RunScenario("peer", context =>
        {
            var controller = context.Instance.Container.Resolve<CoopBattleController>(
            ...
            int guardCommandsBeforeNativeTicks =
                puppetMirror.SetWeaponGuardCalls;
            int guardResetsBeforeNativeTicks =
                puppetMirror.ResetGuardCalls;
            for (int nativeTick = 0; nativeTick < 3; nativeTick++)
            {
                ...
                Assert.Equal(Agent.GuardMode.Right, puppetMirror.GuardMode);
                Assert.Equal(
                    guardCommandsBeforeNativeTicks + nativeTick + 1,
                    puppetMirror.SetWeaponGuardCalls);
                Assert.Equal(
                    guardResetsBeforeNativeTicks,
                    puppetMirror.ResetGuardCalls);
                Assert.Equal(0, puppetMirror.SetActionChannelCalls);
                Assert.Equal(0, puppetMirror.InstallRawVisualActionCalls);
                Assert.Equal(0, puppetMirror.InstallAgentVisualActionCalls);
    ...
    [Fact]
    public void ReceivedMountedReaction_ClearedBeforeDisplay_RefreshesAfterTransition()
    {
        RunScenario("peer", context =>
        {
            var controller = context.Instance.Container.Resolve<CoopBattleController>(
    ...
    [Fact]
    public void MissionPhases_MountedGuardModeDrift_RefreshesAfterMovementWithoutReplayingAction()
    {
        RunScenario("peer", context =>
        {
            var controller = context.Instance.Container.Resolve<CoopBattleController>(
    ...
    [Fact]
    public void MissionTick_MountedGuardRefreshRunsAfterContinuousStateReplay()
    {
        RunScenario("peer", context =>
        {
            var controller = context.Instance.Container.Resolve<CoopBattleController>(
            ...
            ownerMirror.MovementFlags =
                ...
                | Agent.MovementControlFlag.DefendBlock
                | Agent.MovementControlFlag.DefendRight;
            ownerMirror.MovementDirection = new Vec2(1f, 0f);
            ownerMirror.LookDirection = new Vec3(1f, 0f, 0f);
            ownerMirror.InputVector = new Vec2(0f, 1f);
            ownerMirror.Action1Index = 3070;
            ownerMirror.Action1CodeType =
                Agent.ActionCodeType.DefendRight1h;
            ownerMirror.Action1Direction =
            ...
            ApplyOwnerAction(context.Component, 1L, agentId, owner);
            context.Component.AgentActionHandler.ApplyRemoteGuardStates();
            int guardCommandsBeforeMovement =
                puppetMirror.SetWeaponGuardCalls;
            int actionCommandsBeforeMovement =
                puppetMirror.SetActionChannelCalls;
            ...
            Assert.Equal(
                "set-guard",
                puppetMirror.ActionAndGuardCallOrder[
                    puppetMirror.ActionAndGuardCallOrder.Count - 1]);
            Assert.Equal(
                guardCommandsBeforeMovement + 1,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
    ...
    public void MissionTick_MountedGuardTransition_ReplaysActionOnceAfterMovementGuardRefresh()
    {
        RunScenario("peer", context =>
        {
            ...
            ownerMirror.MovementFlags =
                ...
                | Agent.MovementControlFlag.DefendLeft;
            ownerMirror.Action1Index = 3078;
            ownerMirror.Action1Progress = 0.8f;
            ownerMirror.Action1Flags =
                AnimFlags.amf_priority_defend | AnimFlags.anf_cyclic;
            ownerMirror.Action1CodeType =
                Agent.ActionCodeType.DefendLeft1h;
            ...
            int actionIndex =
                puppetMirror.ActionAndGuardCallOrder.LastIndexOf(
                    "set-action");
            Assert.True(continuousStateIndex >= 0);
            Assert.True(guardIndex > continuousStateIndex);
            Assert.True(actionIndex > guardIndex);
            Assert.Equal(
            ...
            Assert.Equal(
                ...
                puppetMirror.ResetGuardCalls);
            Assert.Equal(
                actionCommandsBeforeMovement + 1,
                puppetMirror.SetActionChannelCalls);
            Assert.Equal(3070, puppetMirror.Action1Index);
            Assert.Equal(3070, puppetMirror.SkeletonAction1Index);
            Assert.True(puppetMirror.LastSetActionIgnorePriority);
    ...
    public void AgentActionData_MountedGuardTransition_LeavesGuardCommandAfterMovementFlags()
    {
        RunScenario("peer", context =>
        {
            ...
            bool transitionApplied = new AgentActionData(owner).Apply(
                puppet,
                context.Instance.Container
                    .Resolve<IAgentVisualActionAccessor>());

            Assert.True(transitionApplied);
    ...
    public void MissionPreDisplayTick_MountedGuardVisualTimeline_RemainsNative()
    {
        RunScenario("peer", context =>
        {
            ...
            Assert.Equal(
                guardCommandsBeforeNativeTick + 1,
                puppetMirror.SetWeaponGuardCalls);
            Assert.Equal(
