...
public class MovementTrafficTests : MissionTestEnvironment
{
    ...
    public void PollMovement_PrioritizesFieldBattlePlayerAtSixtyHertz()
    {
        ...
        peer.Call(() =>
        {
            ...
            Assert.True(AgentMirror.TryGet(ai, out MirrorAgent aiMirror));
            Assert.True(AgentMirror.TryGet(player, out MirrorAgent playerMirror));
            ...
            playerMirror.Position = new Vec3(1f, 0f, 0f);
            ...
            network.NetworkSentPackets.Packets.Clear();
            aiMirror.Position = new Vec3(1f, 0f, 0f);
            playerMirror.Position = new Vec3(2f, 0f, 0f);
            component.AgentMovementHandler.PollMovement(0.05f);
            playerMirror.Position = new Vec3(3f, 0f, 0f);
    ...
    [Fact]
    public void PollPlayerMovement_PreservesSixtyHertzAtThirtyOneFps()
    {
        using var fixture = new MissionEngineFixture();
        var peer = Clients.First();
        SetControllerId(peer, "peer");
        ...
        peer.Call(() =>
        {
            ...
            {
                component.AgentMovementHandler.PollMovement(1f / 31f);
                component.AgentMovementHandler.PollPlayerMovement();
            }

            MovementTrafficSnapshot traffic =
                component.AgentMovementHandler.GetTrafficSnapshot();
            Assert.InRange(traffic.PlayerPolls, 60, 61);
        });
    }
