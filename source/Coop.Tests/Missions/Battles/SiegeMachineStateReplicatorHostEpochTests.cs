...
public class SiegeMachineStateReplicatorHostEpochTests : IDisposable
{
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void OwnerRelease_HandsBackAuthority_AndOnlyCrewGrantStartsCooldown(bool proximityGrant)
    {
        const int machineId = 19;
        session.SetupGet(s => s.IsLocalHost).Returns(true);
        InvokePrivate("RefreshMachineCache");
        InvokeSetMachineAuthority(machineId, "remote-a");
        var granted = Assert.IsType<NetworkSiegeMachineAuthority>(Assert.Single(sentToAll));
        if (proximityGrant) ProximityGrants().Add(machineId);
        sentToAll.Clear();

        broker.Publish(this, new NetworkSiegeMachineClaim(machineId, "remote-b", isRelease: true));
        DrainGameThread();
        AssertSingleClaim(machineId, "remote-a");
        Assert.Empty(sentToAll);

        broker.Publish(this, new NetworkSiegeMachineClaim(machineId, "remote-a", isRelease: true));
        DrainGameThread();
        Assert.Empty(ClaimedMachines());
        Assert.DoesNotContain(machineId, ProximityGrants());
        var cooldown = GetField<Dictionary<int, float>>("regrantCooldown");
        Assert.Equal(proximityGrant, cooldown.ContainsKey(machineId));
        if (proximityGrant) Assert.True(cooldown[machineId] > 0f);
        var handback = Assert.IsType<NetworkSiegeMachineAuthority>(Assert.Single(sentToAll));
        Assert.True(string.IsNullOrEmpty(handback.ControllerId));
        Assert.Equal(granted.AuthorityRevision + 1, handback.AuthorityRevision);
        Assert.Equal(LocalEpoch, handback.HostEpoch);

        broker.Publish(this, new NetworkSiegeMachineClaim(machineId, "remote-a", isRelease: true));
        DrainGameThread();
        Assert.Single(sentToAll);
    }
