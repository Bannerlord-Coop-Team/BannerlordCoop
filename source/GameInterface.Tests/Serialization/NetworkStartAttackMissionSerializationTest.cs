...
public class NetworkStartAttackMissionSerializationTest
{
    ...
    [Fact]
    public void LegacyPayload_LeavesMissionInitializerAbsent()
    {
        var legacy = new LegacyNetworkStartAttackMission
        {
            RandomTerrainSeed = 4242,
            MapEventId = "map-event-1",
            AtmosphereOnCampaign = new AtmosphereInfo
            {
                TimeInfo = new TimeInformation { TimeOfDay = 14.0f },
            },
            InitiatingPartyId = "player-party-1",
        };

        byte[] bytes;
        using (var ms = new MemoryStream())
        {
            RuntimeTypeModel.Default.Serialize(ms, legacy);
            bytes = ms.ToArray();
        }
        ...
        using (var ms = new MemoryStream(bytes))
        {
            result = (NetworkStartAttackMission)RuntimeTypeModel.Default.Deserialize(
                ms,
                null,
                typeof(NetworkStartAttackMission));
        }

        Assert.Equal(4242, result.RandomTerrainSeed);
        Assert.Equal("map-event-1", result.MapEventId);
        Assert.Equal(14f, result.AtmosphereOnCampaign.TimeInfo.TimeOfDay);
        Assert.Equal("player-party-1", result.InitiatingPartyId);
        Assert.Null(result.MissionInitializer.SceneName);
    }

    [ProtoBuf.ProtoContract]
    private sealed class LegacyNetworkStartAttackMission
    {
        [ProtoBuf.ProtoMember(1)]
        public int RandomTerrainSeed { get; set; }

        [ProtoBuf.ProtoMember(2)]
        public string MapEventId { get; set; }

        [ProtoBuf.ProtoMember(3)]
        public AtmosphereInfo AtmosphereOnCampaign { get; set; }
