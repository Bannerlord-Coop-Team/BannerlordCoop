...
public class DynamicSyncRegistry
{
    ...
    public void AddField(FieldInfo field, bool debug = false)
    {
        ...
        if (!AddMember(field.DeclaringType, field, debug)) throw new ArgumentException($"{nameof(DynamicSyncBuilder)} Field: {field.Name} has already been registered as a synced field");
    }
    ...
    public void AddProperty(PropertyInfo property, bool debug = false)
    {
        ...
        if (!AddMember(property.DeclaringType, property, debug)) throw new ArgumentException($"{nameof(DynamicSyncBuilder)} Property: {property.Name} has already been registered as a synced property");
    }
