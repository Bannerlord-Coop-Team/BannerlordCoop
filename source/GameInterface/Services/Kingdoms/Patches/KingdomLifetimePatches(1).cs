...
internal class KingdomLifetimePatches
{
	...
	//[HarmonyPatch(typeof(Kingdom), "Remove method name here!")]
	[HarmonyPrefix]
	private static bool RemovePrefix(ref Kingdom __instance)
	{
	// Call original if we call this function
	if (CallOriginalPolicy.IsOriginalAllowed()) return true;
	...
	MessageBroker.Instance.Publish(__instance, new KingdomDestroyed(__instance));
