...
internal sealed partial class LordBarterHandler : IHandler
{
    ...
    private void ProcessRequest(NetPeer peer, NetworkRequestLordBarter request)
    {
        ...
        try
        {
            ...
            // Clan._kingdom is AutoSynced, but the Kingdom._clans / fief collections are not
            // reliably intercepted, so clients would see the clan claim the kingdom while the
            // kingdom's own roster still omitted it. Every other membership mutation in the repo
            // republishes explicitly (see VassalServiceHandler.ApplyVassalage).
