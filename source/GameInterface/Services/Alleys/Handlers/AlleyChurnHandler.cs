...
/// <summary>
/// Runs the vanilla alley AI churn server-authoritatively and drives the AI-initiated attack/defense
/// flow. The gated vanilla handlers publish local triggers (see AlleyCampaignBehaviorPatches); the
/// server applies them over the CoopSession-tracked player alleys (the host's own
/// <c>_playerOwnedCommonAreaData</c> is empty) with patches live, so each side effect - owner change,
/// garrison edit, overseer XP, destroy, attack spawn - replicates through the existing sync, and RNG is
/// rolled once here, never per client. An attack is sent to the owning client, which fights it locally
/// (like the take-over) and reports the result back for the server to adjudicate; an unanswered attack
/// times out in the daily sim.
/// </summary>
internal class AlleyChurnHandler : IHandler
{
    private static readonly ILogger Logger = LogManager.GetLogger<AlleyChurnHandler>();
    ...
    private void Handle_DailyTick(MessagePayload<AlleyDailyTickTriggered> payload)
    {
        ...
        foreach (var alley in behaviorInterface.GetPlayerOwnedAlleys())
        {
            ...
            // Isolate per-alley failures so one bad alley doesn't skip the rest of the day's churn.
            try
            {
                RunDailyAlleyChurn(alley, alleyId, data);
            }
            catch (Exception e)
            {
                Logger.Error(e, "Alley daily churn failed for {alley}", alleyId);
            }
        }
    }

    private void RunDailyAlleyChurn(Alley alley, string alleyId, AlleyManagementData data)
    {
    ...
    private void Handle_DailyTickSettlement(MessagePayload<AlleyDailyTickSettlementTriggered> payload)
    {
        ...
        foreach (var notable in settlement.Notables)
        {
            ...
            {
                Logger.Error(e, "Alley gang churn failed for {notable}", notable.StringId);
            }
        }
    }
    ...
    /// (the host's <c>_playerOwnedCommonAreaData</c> is empty). Each SetOwner replicates via AlleyHandler.
    /// </summary>
    ...
    /// victim) every gang alley it owned is freed. A dead overseer isn't handled here - the daily sim
