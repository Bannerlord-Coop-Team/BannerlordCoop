...
public class CoopTroopSupplier : IMissionTroopSupplier
{
...
#if DEBUG
    /// <summary>One coherent read-only reserve view for a baseline diagnostic.</summary>
    internal sealed class DebugSnapshot
    {
        public bool Populated { get; }
        public string PlayerPartyId { get; }
        public int ReserveRevision { get; }
        public long AllocationRevision { get; }
        public int BattleSize { get; }
        public int SideTotalTroops { get; }
        public int EntryCount { get; }
        public int SuppliedCount { get; }
        public int RemainingCount { get; }
        public IReadOnlyList<DebugPartySnapshot> Parties { get; }
        ...
        internal DebugSnapshot(
            ...
            int suppliedCount,
            int remainingCount,
            IReadOnlyList<DebugPartySnapshot> parties)
        {
            Populated = populated;
    ...
    internal readonly struct DebugPartySnapshot
    {
        ...
        internal DebugPartySnapshot(string partyId, int entryCount, int suppliedCount, int remainingCount)
        {
            PartyId = partyId;
            EntryCount = entryCount;
            SuppliedCount = suppliedCount;
    ...
    internal DebugSnapshot CaptureDebugSnapshot()
    {
        lock (gate)
        {
            var snapshots = new List<DebugPartySnapshot>(parties.Count);
            int entries = 0;
            int supplied = 0;
            foreach (var party in parties)
            {
                int entryCount = party.Entries.Length;
                int suppliedCount = party.Supplied;
                int remainingCount = entryCount - suppliedCount;
                snapshots.Add(new DebugPartySnapshot(
                    party.PartyId,
                    entryCount,
                    suppliedCount,
                    remainingCount));
