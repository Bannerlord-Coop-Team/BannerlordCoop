...
#if DEBUG
    private static WorldMapLagFixture worldMapLagFixture;

    private sealed class WorldMapLagFixture
    {
        public string BattleControllerId { get; set; }
        public string MapControllerId { get; set; }
        public string BattlePartyId { get; set; }
        public string MapPartyId { get; set; }
...
#if DEBUG
    [CommandLineArgumentFunction("world_map_lag_capture", "coop.debug.mapevent")]
    public static string CaptureWorldMapLagFixture(List<string> args)
    {
        if (ModInformation.IsClient)
            return "Run this command on the server.";
        if (args.Count != 2)
            return "Usage: coop.debug.mapevent.world_map_lag_capture <battleControllerId> <mapControllerId>";
        if (worldMapLagFixture?.Started == true || lateJoinModeFixture != null)
            return JsonResult(new { ok = false, error = "A field-battle fixture is already active." });
        if (args[0] == args[1])
            return JsonResult(new { ok = false, error = "Two different connected players are required." });

        if (!TryGetPlayerParty(args[0], requireReady: true, out var objectManager, out var battleParty, out var error) ||
            !TryGetPlayerParty(args[1], requireReady: true, out _, out var mapParty, out error))
        {
            return JsonResult(new { ok = false, error });
        }
        if (battleParty.CurrentSettlement != null || mapParty.CurrentSettlement != null)
            return JsonResult(new { ok = false, error = "Both players must be on the campaign map outside settlements." });
        if (battleParty.MapEvent != null || mapParty.MapEvent != null)
            return JsonResult(new { ok = false, error = "Both players must be outside map events." });
        if (!objectManager.TryGetId(battleParty, out var battlePartyId) ||
            !objectManager.TryGetId(mapParty, out var mapPartyId))
        {
            return JsonResult(new { ok = false, error = "Unable to resolve both player party ids." });
        }

        worldMapLagFixture = new WorldMapLagFixture
        {
            BattleControllerId = args[0],
            MapControllerId = args[1],
            BattlePartyId = battlePartyId,
            MapPartyId = mapPartyId,
        };
        ...
        return JsonResult(new
        {
            ...
            battlePartyId,
            mapPartyId,
            bothOnCampaignMap = battleParty.MapEvent == null && mapParty.MapEvent == null,
        });
    }

    [CommandLineArgumentFunction("world_map_lag_start", "coop.debug.mapevent")]
    public static string StartWorldMapLagFixture(List<string> args)
    {
        if (ModInformation.IsClient)
            return "Run this command on the server.";
        if (args.Count != 2)
            return "Usage: coop.debug.mapevent.world_map_lag_start <battleControllerId> <mapControllerId>";

        var fixture = worldMapLagFixture;
        if (fixture == null || fixture.BattleControllerId != args[0] || fixture.MapControllerId != args[1])
            return JsonResult(new { ok = false, error = "Capture this exact player pair before starting the fixture." });
        if (fixture.Started)
            return JsonResult(new { ok = false, error = "The world-map lag fixture was already started." });

        var result = StartLateJoinModeFixture(args);
        fixture.Started = lateJoinModeFixture != null;
        return JsonResult(new
        {
            ok = fixture.Started,
            phase = fixture.Started ? "started" : "failed",
            fixture.MapControllerId,
            fixture.BattleControllerId,
            mapEventId = lateJoinModeFixture?.MapEventId,
            detail = result,
        });
    }

    [CommandLineArgumentFunction("world_map_lag_state", "coop.debug.mapevent")]
    public static string GetWorldMapLagState(List<string> args)
    {
        if (args.Count != 0)
            return "Usage: coop.debug.mapevent.world_map_lag_state";
        ...
        if (ModInformation.IsClient)
        {
            ...
            return JsonResult(new
            {
                ok = true,
                role = "client",
                missionActive = MissionState.Current != null || mission != null,
                deploymentActive = mission?.HasMissionBehavior<DeploymentHandler>() == true,
                missionAgents = mission?.Agents.Count ?? 0,
                activeState = GameStateManager.Current?.ActiveState?.GetType().Name ?? "none",
                mapEventActive = MobileParty.MainParty?.MapEvent != null,
        ...
        var fixture = worldMapLagFixture;
        if (fixture == null)
            return JsonResult(new { ok = false, role = "server", error = "No world-map lag fixture was captured." });

        MobileParty battleParty = null;
        MobileParty mapParty = null;
        TryGetObjectManager(out var objectManager);
        objectManager?.TryGetObject(fixture.BattlePartyId, out battleParty);
        objectManager?.TryGetObject(fixture.MapPartyId, out mapParty);
        ...
        return JsonResult(new
        {
            ok = true,
            role = "server",
            fixtureActive = lateJoinModeFixture != null,
            fieldBattle = mapEvent?.EventType == MapEvent.BattleTypes.FieldBattle && mapEvent.MapEventSettlement == null,
            battlePlayerInMission = missionMembership?.IsControllerInMission(fixture.BattleControllerId) == true,
            mapPlayerInMission = missionMembership?.IsControllerInMission(fixture.MapControllerId) == true,
            battlePlayerMapEvent = battleParty?.MapEvent != null,
            mapPlayerMapEvent = mapParty?.MapEvent != null,
            campaignTicks = CampaignTime.Now.NumTicks,
        });
    }

    [CommandLineArgumentFunction("world_map_lag_restore", "coop.debug.mapevent")]
    public static string RestoreWorldMapLagFixture(List<string> args)
    {
        if (ModInformation.IsClient)
            return "Run this command on the server.";
        if (args.Count > 1)
            return "Usage: coop.debug.mapevent.world_map_lag_restore [capturedStateJson]";

        var fixture = worldMapLagFixture;
        if (fixture == null)
            return JsonResult(new { ok = false, error = "No world-map lag fixture was captured." });

        var exitDetail = lateJoinModeFixture == null
            ? "No fixture mission was active."
            : ExitLateJoinModeFixtureMissions(new List<string>());
        var cleanupDetail = lateJoinModeFixture == null
            ? "No field-battle fixture was active."
            : CleanupLateJoinModeFixture(new List<string>());
        fixture.Restored = lateJoinModeFixture == null;
        if (fixture.Restored)
        ...
        {
            ok = fixture.Restored,
            phase = fixture.Restored ? "restored" : "restore-failed",
            exitDetail,
            cleanupDetail,
        });
    }

    [CommandLineArgumentFunction("world_map_lag_verify", "coop.debug.mapevent")]
    public static string VerifyWorldMapLagFixture(List<string> args)
    {
        if (ModInformation.IsClient)
            return "Run this command on the server.";
        if (args.Count != 0)
            return "Usage: coop.debug.mapevent.world_map_lag_verify";
        ...
        var battlePlayerInMission = missionMembership?.IsControllerInMission(fixture.BattleControllerId) == true;
        var mapPlayerInMission = missionMembership?.IsControllerInMission(fixture.MapControllerId) == true;
        var restored = fixture.Restored && lateJoinModeFixture == null &&
                       battleParty?.MapEvent == null && mapParty?.MapEvent == null &&
                       !battlePlayerInMission && !mapPlayerInMission;

        return JsonResult(new
        {
            ok = restored,
            phase = "verified",
            fixtureRemoved = lateJoinModeFixture == null,
            battlePlayerMapEvent = battleParty?.MapEvent != null,
            mapPlayerMapEvent = mapParty?.MapEvent != null,
            battlePlayerInMission,
            mapPlayerInMission,
        });
    }
