...
internal class ClanPatches
{
    ...
    [HarmonyPatch(nameof(Clan.EnterKingdomInternal))]
    [HarmonyPostfix]
    public static void EnterKingdomInternalPostfix(Clan __instance)
    {
    ...
    [HarmonyPatch(nameof(Clan.LeaveKingdomInternal))]
    [HarmonyPrefix]
    public static void LeaveKingdomInternalPrefix(Clan __instance)
    {
        if (ModInformation.IsClient) return;
