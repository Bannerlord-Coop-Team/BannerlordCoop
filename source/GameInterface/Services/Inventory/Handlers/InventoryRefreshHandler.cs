...
internal class InventoryRefreshHandler : IHandler
{
    ...
    private void Handle_RefreshOtherInventory(MessagePayload<RefreshOtherInventory> obj)
    {
        ...
        {
            UpdateItemCosts(obj.What.EquipmentElements);
            return;
        }
        ...
        GameLoopRunner.RunOnMainThread(() =>
        {
            ...
            foreach (var equipmentElement in obj.What.EquipmentElements)
            {
                int itemRosterElementIndex = currentInventoryVM._inventoryLogic.GetElementsInRoster(InventoryLogic.InventorySide.OtherInventory).FindIndex(rosterElement => rosterElement.EquipmentElement.Equals(equipmentElement));
                ...
                {
                    if (itemVM.ItemRosterElement.EquipmentElement.IsEqualTo(equipmentElement)) // Different amounts, can't compare ItemRosterElement directly
                    {
                        targetItemVM = itemVM;
                        break;
                    }
    ...
    private void UpdateItemCosts(HashSet<EquipmentElement> equipmentElements)
    {
        GameLoopRunner.RunOnMainThread(() =>
        {
    ...
    private void Handle_RefreshAfterTrade(MessagePayload<RefreshAfterTrade> obj)
    {
        ...
        // Don't update if the client isn't looking at the roster involved in the trade
        if (fromItemRoster != currentInventoryVM._inventoryLogic._rosters[0] && toItemRoster != currentInventoryVM._inventoryLogic._rosters[0]) return;
