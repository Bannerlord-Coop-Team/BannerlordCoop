...
/// </summary>
internal class SettlementAuditor : AuditorBase<RequestSettlementAudit, SettlementAuditResponse, SettlementAuditData, Settlement>
{
    ...
    protected override string AuditData(IEnumerable<SettlementAuditData> data)
    {
        var typeName = nameof(Settlement);
        var sb = new StringBuilder();

        var errorCount = 0;
        var objectsCount = Objects.Count();
        sb.AppendLine($"Auditing {objectsCount} objects");

        if (objectsCount != data.Count())
        {
            Logger.Error($"{typeName} count mismatch: {objectsCount} != {data.Count()}");
            sb.AppendLine($"{typeName} count mismatch: {objectsCount} != {data.Count()}");
        }
        ...
        foreach (var audit in data)
        {
            if (objectManager.TryGetObject<Settlement>(audit.StringId, out var settlement) == false)
            {
                ...
                sb.AppendLine($"{typeName} {audit} not found in {nameof(IObjectManager)}");
                errorCount++;
            }
            else
            {
        ...
        sb.AppendLine($"Found {errorCount} errors in {data.Count()} objects");
        return sb.ToString();
    }
}
