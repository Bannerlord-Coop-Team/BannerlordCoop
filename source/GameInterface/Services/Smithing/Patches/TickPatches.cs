...
internal class TickPatches
{
    ...
    [HarmonyPrefix]
    public static bool DailyTickSettlementPrefix(ref CraftingCampaignBehavior __instance, Settlement settlement)
    {
        // Call original if we call this function
        if (CallOriginalPolicy.IsOriginalAllowed()) return true;
        ...
        ContainerProvider.TryResolve<ICraftingCampaignBehaviorInterface>(out var craftingCampaignBehaviorInterface);
        craftingCampaignBehaviorInterface.DailyTickSettlement(__instance, settlement);
    ...
    [HarmonyPrefix]
    public static bool DailyTickPrefix(ref CraftingCampaignBehavior __instance)
    {
        // Call original if we call this function
        if (CallOriginalPolicy.IsOriginalAllowed()) return true;
