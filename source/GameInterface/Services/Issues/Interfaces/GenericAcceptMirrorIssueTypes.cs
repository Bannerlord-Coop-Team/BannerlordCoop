...
internal static class GenericAcceptMirrorIssueTypes
{
    internal static readonly HashSet<Type> QuestSolutionMirrorEligible = new HashSet<Type>
    {
        ...
        // derived value every other type here already relies on. A bare replay of IssueManager.StartIssueQuest
        // lands on a byte-identical SmugglersIssueQuest on every peer. The one genuinely per-client-divergent
        // piece of this quest - _smugglerParty, only ever set later by the dialogue-triggered
        // QuestAcceptedConsequences, not by this replay - is handled entirely separately by
        // SmugglersPartySpawnGatePatch, not by this generic accept mirror.
        ...
        // lands on a byte-identical ArtisanOverpricedGoodsIssueQuest on every peer. The one genuinely bespoke
        // mechanic it needs (the AntagonistHero/CounterOfferHero identity fix) is a pure, local,
        // network-message-free Harmony patch entirely orthogonal to accept-time mirror eligibility - see
        // Patches.ArtisanOverpricedGoodsAntagonistIdentityPatches.
    ...
    internal static readonly HashSet<Type> AlternativeSolutionMirrorEligible = new HashSet<Type>
    {
        ...
        // ownership-gated HourlyTick mechanism every other type here uses - see
        ...
        // Casualties | FailureRisk (confirmed against the decompiled source) - genuinely can fail, and is still
        // safe to route through this same generic trigger (see NewIssueTypesAlternativeSolutionCompletionPatches'
        // doc comment). Needs its matching HourlyTick registration too - see that file's SmugglersIssueBehavior
        // entry.
        typeof(SmugglersIssueBehavior.SmugglersIssue),
