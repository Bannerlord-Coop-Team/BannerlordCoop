...
internal class AutoSync : IAutoSync
{
    ...
    public ISyncResults SyncField<T>(FieldInfo fieldInfo)
    {
...
public class FieldSync : IDisposable
{
    private static readonly Dictionary<FieldInfo, ISyncResults> PatchedFields = new();
    ...
    public ISyncResults SyncField<T>(FieldInfo field) where T : class
    {
        if (PatchedFields.TryGetValue(field, out var existingResult))
        {
            return existingResult;
        }
