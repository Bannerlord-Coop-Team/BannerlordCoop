...
namespace Coop.IntegrationTests.Towns
{
    public class TownLastCapturedByTest
    {
        ...
        /// <summary>
        /// Used to Test that client recieves TownLastCapturedByChanged messages.
        /// </summary>
        [Fact]
        public void ServerTownLastCapturedByChanged_Publishes_AllClients()
        {
            ...
            Assert.Equal(1, server.NetworkSentMessages.GetMessageCount<NetworkChangeTownLastCapturedBy>());

            // Verify the all clients send a single message to their game interfaces
            foreach (EnvironmentInstance client in TestEnvironment.Clients)
            {
                Assert.Equal(1, client.InternalMessages.GetMessageCount<ChangeTownLastCapturedBy>());
            }
        }
    }
}
