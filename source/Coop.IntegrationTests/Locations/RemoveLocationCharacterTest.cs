...
{
    public class RemoveLocationCharacterTest
    {
        // Creates a test environment with 1 server and 2 clients by default
        internal TestEnvironment TestEnvironment { get; } = new TestEnvironment();

        /// <summary>
        /// Used to Test that client receives RemoveLocationCharacter messages.
        /// </summary>
        [Fact]
        public void ServerLocationCharacterRemoved_Publishes_AllClients()
        {
            // Arrange
            ...
            var triggerMessage = new LocationCharacterRemoved(location, character);
            ...
            Assert.Equal(1, server.NetworkSentMessages.GetMessageCount<NetworkRemoveLocationCharacter>());

            // Verify all clients receive a single message to their game interfaces
            foreach (EnvironmentInstance client in TestEnvironment.Clients)
            {
                Assert.Equal(1, client.InternalMessages.GetMessageCount<NetworkRemoveLocationCharacter>());
            }
        }
    }
}
