...
namespace Coop.IntegrationTests.Armies
{
    public class AddMobilePartyArmyTest
    {
        ...
        public void ServerMobilePartyInArmyAdded_Publishes_AllClients()
        {
            ...
            Assert.Equal(1, server.NetworkSentMessages.GetMessageCount<NetworkChangeAddMobilePartyInArmy>());

            //Verify if the server is sending the same mobilePartyId and leaderMobilePartyId value
            Assert.Equal(mobilePartyId, server.NetworkSentMessages.GetMessages<NetworkChangeAddMobilePartyInArmy>().First().MobilePartyId);
            Assert.Equal(leaderMobilePartyId, server.NetworkSentMessages.GetMessages<NetworkChangeAddMobilePartyInArmy>().First().LeaderMobilePartyId);
