...
/// </summary>
public class SessionJoinWatchdog : IHandler
{
    private static readonly ILogger Logger = LogManager.GetLogger<SessionJoinWatchdog>();
    ...
    public SessionJoinWatchdog(IMessageBroker messageBroker)
    {
    ...
    private void OnTimeout()
    {
        ...
        GameThread.RunSafe(() =>
        {
            ...
            string popupText = timeoutText ?? (tunneled
                ? "Could not reach the co-op host through the storefront network. Make sure the host is still in their session, then try the invite again."
