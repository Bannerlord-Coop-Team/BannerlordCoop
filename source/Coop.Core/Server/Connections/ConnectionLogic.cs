...
public class ConnectionLogic : IConnectionLogic
{
    ...
    private IReadOnlyDictionary<Type, Func<IConnectionState>> CreateStateFactories() =>
        new Dictionary<Type, Func<IConnectionState>>
        {
            ...
            [typeof(TransferSaveState)] = () => new TransferSaveState(this, context.Network, context.CoopSessionProvider, context.SaveInterface, context.TimeControlInterface, context.ConnectionMessageQueue, context.Coalescer, context.AttachmentIdMapper, context.BattleSizeProvider),
