...
The response includes the artifact directory and the same structured report. Input/idle/initial disk/lease failures can be MCP errors before an attempt directory is created. Build and subsequent failures return a report with `failed_before_apply`, `rolled_back`, or `rollback_failed`; success is `deployed`. A pre-crash manifest may show `preparing`, `backup_verified` or `applying`, none of which proves completion.
